...
unsigned
cost_of_balloons::calculateMinTotalPrice(vector<UserStatus> statuses,
  ...
  auto prizes_for_first =
      count_if(begin(statuses), end(statuses),
               [](auto status) { return status.first_solved; });
  auto prizes_for_second =
      count_if(begin(statuses), end(statuses),
               [](auto status) { return status.second_solved; });
